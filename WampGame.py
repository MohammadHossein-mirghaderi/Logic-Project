...
class Wampus:
    ...
    def sound_warnings(self, neighbors):
        warnings = []
        positions = {
            self.bat_1_pos: Wampus_inf.BAT_WARNING,
            self.pit_1_pos: Wampus_inf.PIT_WARNING,
            self.wampus_pos: Wampus_inf.WUMPUS_WARNING
        }
        [warnings.append(positions.get(neighbor)) for neighbor in neighbors if
         positions.get(neighbor, "nothing") != "nothing"]
        warnings = list(dict.fromkeys(warnings))
        return warnings

    # update_positions : Game Nat -> ???
    # Updates the hero position and the explored positions list 
    def updating(self, new_position):
        ...
        if self.hero_pos == self.bat_1_pos:
            ...
            self.updating(choice(change_list))
    ...
    def is_new_pos_valid(self, direction_value):
        ...
        new_pos = Pos(new_dir)
        return new_dir < Wampus_inf.TOTAL_LENGTH and new_dir >= 0 and (
                    old_pos.row == new_pos.row or old_pos.column == new_pos.column)

    # check_wampus_state : Game Nat -> None
    # Checks to see if the wampus is dead.
    def check_wampus_state(self, key_pressed):
        arrow_dict = {
            ord('w'): Wampus_inf.up_direction,
            ord('s'): Wampus_inf.down_direction,
            ord('a'): Wampus_inf.left_direction,
            ord('d'): Wampus_inf.right_direction
        }
...
class Pos:
    def __init__(self, num):
        self.row = int(num / Wampus_inf.ROW_LENGTH)
        self.column = num % Wampus_inf.ROW_LENGTH
...
def show_rules(s):
    ...
    s.addstr(0, 0, " " * buffer_length)
    s.addstr(0, buffer_length, exit_statement)
    s.addstr(0, start_end_buffer, " " * end_buffer)
    s.attroff(curses.color_pair(6))
    for counter, line in enumerate(Wampus_inf.Instructions):
        x_pos = width // 2 - len(line) // 2 - len(line) % 2
        y_pos = height // 2 - len(Wampus_inf.Instructions) // 2 + counter
        s.addstr(y_pos, x_pos, line)
    s.refresh()
    exit_menu = False
...
def menu(stdscr):
    ...
    while in_menu:
        ...
        if key == curses.KEY_UP and current_row > 0:
            current_row -= 1
        elif key == curses.KEY_DOWN and current_row < len(Wampus_inf.menu) - 1:
            current_row += 1
        ...
        for j in range(5):
            if len(discovered) > 0 and counter == discovered[0]:
                if counter == hero_pos:
                    ...
                    row += Wampus_inf.DISCOVERED
                discovered = discovered[1:]
            else:
                row += Wampus_inf.UNDISCOVERED
            counter += 1
...
def coords_to_pos(row, col):
    return Wampus_inf.ROW_LENGTH * row + col
...
def get_neighbors(hero_pos):
    ...
    for i in range(row_low_range, row_high_range):
        for j in range(col_low_range, col_high_range):
            if i >= Wampus_inf.ROW_LENGTH or i < 0 or j >= Wampus_inf.COLUMN_LENGTH or j < 0 or i == hero_row and j == hero_col:
                pass
            else:
...
def main(stdscr):
    ...
    while key_pressed != ord('q'):
        ...
        if game.game_over == True:
            stdscr.attron(curses.color_pair(4))
            stdscr.addstr(int(height // 2),
                          int(width // 2 - len(game.reason_for_game_end) // 2 - len(game.reason_for_game_end) % 2),
                          game.reason_for_game_end)
            stdscr.addstr(int(height // 2) + 4,
                          int(width // 2 - len(Wampus_inf.QUIT_STATEMENT) // 2 - len(Wampus_inf.QUIT_STATEMENT) % 2),
                          Wampus_inf.QUIT_STATEMENT)
            stdscr.attroff(curses.color_pair(4))
            stdscr.refresh()
            ...
            for i in range(Wampus_inf.ROW_LENGTH):
                stdscr.addstr(int(height // 2 - len(assembly) // 2 + i),
                              int(width // 2 - len(assembly[i]) // 2 - len(assembly[i]) % 2), assembly[i])
            stdscr.attroff(curses.color_pair(4))
            ...
            for i in range(len(warnings)):
                stdscr.addstr(int(height // 2 - len(assembly) // 2 + i + len(assembly) + 1),
                              int(width // 2 - len(warnings[i]) // 2 - len(warnings[i]) % 2), warnings[i])
            stdscr.attroff(curses.color_pair(1))

            # Printing the remaining arrows
            stdscr.attron(curses.color_pair(4))
            arrows_remaining = str(game.arrows) + " arrows remain."
            stdscr.addstr(height - 1, int(width // 2 - len(arrows_remaining) // 2 - len(arrows_remaining) % 2),
                          arrows_remaining)
            stdscr.attroff(curses.color_pair(4))
            ...
            if key_pressed == curses.KEY_UP:
                direction_value = Wampus_inf.up_direction
            elif key_pressed == curses.KEY_DOWN:
                direction_value = Wampus_inf.down_direction
            elif key_pressed == curses.KEY_RIGHT:
                direction_value = Wampus_inf.right_direction
            elif key_pressed == curses.KEY_LEFT:
                direction_value = Wampus_inf.left_direction
            elif key_pressed == ord('w') or key_pressed == ord('a') or key_pressed == ord('s') or key_pressed == ord(
                    'd'):
                game.arrows -= 1
            ...
            if direction_value != 0 and game.is_new_pos_valid(direction_value):
                ...
                game.updating(new_pos)


def print_menu(stdscr, selected_row_id):
    stdscr.clear()
    height, width = stdscr.getmaxyx()
    for count, menu_item in enumerate(Wampus_inf.menu):
        x_pos = width // 2 - len(menu_item) // 2 - len(menu_item) % 2
        y_pos = height // 2 - len(Wampus_inf.menu) // 2 + count
...
class Wampus_inf:
    ...
    _INSTRUCTIONS = """Welcome to 'Logic project Wumpus'.
...
Wumpus: The Wumpus is not bothered by hazards as he has sucker feet
